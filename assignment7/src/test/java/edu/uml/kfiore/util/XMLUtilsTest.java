...
import edu.uml.kfiore.xml.XMLDomainObject;
...
public class XMLUtilsTest {
    ...
    private static String STOCK_SYMBOL = "VNET";
    private static String STOCK_PRICE = "110.10";
    ...
    public void testUnmarshall() throws Exception {
        Stock stock = XMLUtils.unmarshall(xmlInstance, Stock.class);
        validateStock(stock);
    }

    @Test
    public void testUnmarshallWithSchemaValidation()throws Exception {
        Stock stock = XMLUtils.unmarshall(xmlInstance, Stock.class, "/xml/stock_info.xsd");
        validateStock(stock);
    }
    ...
    public void testMarshall() throws Exception {
        Stock stock = XMLUtils.unmarshall(xmlInstance, Stock.class, "/xml/stock_info.xsd");
        String xml = XMLUtils.marshall((XMLDomainObject) stock);
        // input xml should be the same as output xml
        assertEquals("XML out is correct", xml.trim() ,xmlInstance.trim());
    }

    private void validateStock(Stock stock) {
        assertTrue("Stock symbol is correct", stock.getSymbol().getValue().equals(STOCK_SYMBOL));
        assertTrue("Stock price is correct", stock.getPrice().getValue().equals(STOCK_PRICE));
        assertTrue("There is a date/time", stock.getTime().getValue().equals(STOCK_TIME));
    }
